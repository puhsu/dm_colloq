...
\begin{document}
	...
	\section*{1. Принцип математической индукции}
        ...
        \end{defin}
        Утверждение $A(1)$ называется базисом индукции (или базой индукции), а условное утверждение если $A(n)$ верно, то $A(n + 1)$ верно -- индуктивным переходом (или шагом индукции). Посылка $A(n)$ называется индуктивным предположением.

    \section*{2. Перечислительная комбинаторика}

        \subsection*{Правило произведения}
            \begin{defin}
                Если $A$ и $B$ конечные множества, то $|A \times B| = |A| \cdot |B|$,
                где $|M|$ -- число элементов множества.
            \end{defin}
